...
import { setCurPage, resetPage } from "../../store/postSlice";
import UserStats from "./UserStats";
//디자인 
import { scrollStyle, cardStyle } from "../../style/CommonStyle";
import { emptyDiv } from "../../style/CommonEmptyBoard";
...
const MyBoard = () => {
  ...
  // 탭 상태 관리
  const [activeTab, setActiveTab] = useState("posts"); // "posts" 또는 "stats"
...
>
  ...
  return (
    ...
    <div ref={scrollRef} className={`${scrollStyle} ${OpenState ? 'max-md:h-[63vh] ' : 'max-md:h-[83vh]'}  md:h-[90vh] mt-1 ml-20 pr-40 max-md:m-1 max-md:p-2 max-md:overflow-x-hidden`}>
      ...
      {/* 탭 UI */}
      <div className="flex border-b border-gray-600 mt-2 mb-4">
        <button
          onClick={() => setActiveTab("posts")}
          className={`px-6 py-3 font-semibold text-lg border-b-2 transition-colors ${
            activeTab === "posts" 
              ? "border-[#C5BCFF] text-[#C5BCFF]" 
